...
# help, feature list, and bundled model catalog exposed by the packaged binary.
{
  ...
  committed = ../packages/chatgpt-codex/extracted.json;
in {
  chatgpt-codex-extracted = pkgs.runCommand "chatgpt-codex-extracted-drift" {} ''
    ...
    if "$jq" -e -n --slurpfile a ${extracted} --slurpfile b ${committed} \
      '$a == $b' > /dev/null; then
      echo "ok — packages/chatgpt-codex/extracted.json matches the packaged binary" > $out
    else
      echo "FAIL: packages/chatgpt-codex/extracted.json is out of sync with the Codex binary." >&2
      echo "--- committed ---" >&2
      "$jq" -S . ${committed} >&2
      ...
      echo "then copy the result over packages/chatgpt-codex/extracted.json, format it, and git add it." >&2
      exit 1
    fi
  '';
}
