# checks/daemon-only-source.nix -- the executing guard for
...
pkgs.runCommand "daemon-only-source" {
  ...
  python3 ${self}/checks/daemon-only-source.py ${self} | tee "$out"
  ...
  if python3 ${self}/checks/daemon-only-source.py "$fixture" \
