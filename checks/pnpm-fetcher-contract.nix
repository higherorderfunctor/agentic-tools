...
# combination. `checks/pnpm-fetcher-parity.nix` could not catch it either: that
...
in {
  pnpm-fetcher-contract = pkgs.runCommand "pnpm-fetcher-contract" {} ''
    ${lib.optionalString tooFew ''
      ...
      echo "retired deliberately, lower the floor in checks/pnpm-fetcher-contract.nix in" >&2
