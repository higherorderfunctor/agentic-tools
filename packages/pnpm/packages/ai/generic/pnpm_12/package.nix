...
# pnpm_11 siblings.
#
# THIS FILE DELIBERATELY DOES NOT USE ../../../../lib/mkMajor.nix, and that is the
...
# and what ../../../../lib/mkMajor.nix overrides the version and `src` of. In 12 the
...
#
#   3. THE MAJOR GUARD IN ../../../../lib/mkMajor.nix IS NOT THE ANSWER HERE. That
...
# one packages/chatgpt-codex/packages/ai/chatgpt-codex/package.nix uses and for the same reason — there is
...
# packages/bun/packages/ai/generic/bun/package.nix's does.
#
...
# through unwrapped.
{
  ...
  platformSrc =
    ...
    then throw "pnpm_12: no source pinned for ${system} in packages/pnpm/sources-12.json"
    else
      fetchurl {
  ...
  # THE MAJOR GUARD, which ../../../../lib/mkMajor.nix carries for 10 and 11 and
  ...
  #
  # The failure mode differs from mkMajor.nix's (there it is
...
in
  ...
  '';
    stdenv.mkDerivation (finalAttrs: {
      ...
      #
      # `packages/chatgpt-codex/packages/ai/chatgpt-codex/package.nix` sets this for the same reason. A
      ...
      #
      # Unlike packages/claude-code/packages/ai/claude-code/package.nix, autoPatchelfHook is SAFE here:
