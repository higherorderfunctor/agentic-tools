...
StrictDoc graph. This branch predates R2's exported document path, so its board
temporarily joins provenance through `uid_paths`; once that stack is combined,
the daemon export supplies the path directly. The semantics payload still reads
the operator-owned model from disk, explicitly recorded under
